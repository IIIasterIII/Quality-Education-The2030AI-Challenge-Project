        ...
        <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/80 backdrop-blur-xl animate-in fade-in duration-300">
            <div className="w-[800px] max-h-[90vh] bg-zinc-950 border border-zinc-800 rounded-3xl overflow-hidden shadow-2xl flex flex-col scale-in-center">
                <div className="p-8 border-b border-zinc-900 flex justify-between items-center">
                    <div>
                        <h2 className="text-2xl font-black italic text-white">Image Wizard</h2>
                        <p className="text-zinc-500 text-sm">Perfect your visual knowledge.</p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-10 flex gap-12">
                    ...
                    >
                        {imgUrl ? (
                            ...
                            <div className="text-zinc-800 text-center space-y-3">
                                <div className="w-16 h-16 bg-zinc-950/50 rounded-full flex items-center justify-center mx-auto mb-4 border border-white/5">
                                    <ImageIcon className="w-8 h-8 opacity-20" />
                                </div>
                                <p className="text-sm font-black uppercase tracking-[0.2em] opacity-40">Drop Image Here</p>
                                <p className="text-xs text-zinc-800">or click to browse</p>
                            </div>
                        )}
                    ...
                    <div className="w-72 space-y-8 flex flex-col">
                        <div className="space-y-3">
                            <label className="text-[10px] uppercase font-black tracking-[0.2em] text-zinc-600">Source Asset</label>
                            <div className="flex gap-2">
                                <Input placeholder="Asset URL" className="bg-zinc-900 border-zinc-800 h-12 text-sm text-zinc-200" value={imgUrl.startsWith('data:') ? 'Local Asset' : imgUrl} onChange={(e) => setImgUrl(e.target.value)} />
                                <Button variant="outline" className="h-12 bg-zinc-950 border-zinc-800" onClick={() => document.getElementById('file-upload-wizard')?.click()}><Plus className="w-4 h-4" /></Button>
                            </div>
                        </div>
                        <div className="space-y-4">
                            <label className="text-[10px] uppercase font-black tracking-[0.2em] text-zinc-600">Rotation • {imgRotation}°</label>
                            <div className="flex gap-2">
